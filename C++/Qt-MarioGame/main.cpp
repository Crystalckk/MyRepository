...
int main(int argc, char *argv[]) {
    ...
    vector<pair<unique_ptr<Entity>, int>> obstacles_level1;
    vector<pair<unique_ptr<Entity>, int>> obstacles_level2;
    stageConfig.obstacles_level1 = &obstacles_level1;
    stageConfig.obstacles_level2 = &obstacles_level2;
    ...
    while (!line.isNull()) {
        ...
        if (setting == "size:") {
        ...
        } else if (setting == "obstacles_level1:") {
            ...
            for (QString &s : parts) {
                ...
                obstacles_level1.push_back(make_pair(move(e), sl.at(3).toInt()));
            }
        } else if (setting == "obstacles_level2:"){
            QStringList parts = value.split("|");
            EntityFactory factory;
            for (QString &s : parts) {
                QStringList sl = s.split(",");
                ...
                // Add the pair (obstacle, spacing_to_next_obstacle) to our obstacle layout
                obstacles_level2.push_back(make_pair(move(e), sl.at(3).toInt()));
            }
        } else if (setting == "testMode:") {
            stageConfig.testMode = value.compare("on") == 0;
        } else if(setting == "numLives:"){
            if (!isNumber(value.toStdString())) {
                cout << "X coordinate must be set to a postive integer. Terminating";
                return 0;
            }
            unsigned int num = value.toUInt();
            stageConfig.numLives = num;
        }
